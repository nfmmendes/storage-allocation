...
void WarehouseToGraphConverter::generateGraph(){
    ...
    map<Corridor, vector<Curve> > curvesByCorridor;
    map<Corridor, vector<Point> > pointsByCorridor;
    ...
    for(int i= 0; i<(int) blocks.size(); i++){
        ...
        for(int j = 0; j<(int)curves.size(); i++){
          //  splitCorridorByCurves(curves[i], curvesByCorridor, pointsByCorridor);
        }
